...
export const propertyDepartments = pgTable(
  ...
  (table) => [
    ...
    unique("property_departments_property_id_department_id_key").on(
      table.propertyId,
      table.departmentId,
    ),
  ],
...
export const headcountProjects = pgTable(
  "headcount_projects",
  {
    id: serial().primaryKey().notNull(),
...
export const propertyValues = pgTable(
  ...
  (table) => [
    ...
    unique("property_values_project_id_property_id_project_type_key").on(
      table.projectId,
      table.propertyId,
      table.projectType,
    ),
...
export const milestoneDependencies = pgTable(
  ...
  (table) => [
    ...
    unique("milestone_dependencies_from_milestone_id_to_milestone_id_key").on(
      table.fromMilestoneId,
      table.toMilestoneId,
    ),
  ],
)
...
export const plans = pgTable(
  ...
  (table) => [
    ...
    unique("plans_project_id_version_name_key").on(
      table.projectId,
      table.versionName,
    ),
    check(
      "plans_status_check",
      sql`(status)::text = ANY ((ARRAY['DRAFT'::character varying, 'ACTIVE'::character varying, 'ARCHIVED'::character varying])::text[])`,
    ),
  ],
)

export const headcountStandards = pgTable(
  "headcount_standards",
  {
    id: serial().primaryKey().notNull(),
    roleId: integer("role_id").notNull(),
    ...
    monthlyFactors: jsonb("monthly_factors")
      .$type<number[]>()
      .default([])
      .notNull(),
    projectType: varchar("project_type", { length: 30 })
  ...
  (table) => [
    ...
    check(
      "headcount_standards_project_type_check",
      sql`(project_type)::text = ANY ((ARRAY['ALL'::character varying, 'LOW_RISE'::character varying, 'HIGH_RISE'::character varying, 'MIXED'::character varying])::text[])`,
    ),
  ],
)
...
export const phases = pgTable(
  ...
  (table) => [
    ...
    unique("phases_plan_id_milestone_id_key").on(
      table.planId,
      table.milestoneId,
    ),
  ],
...
export const headcountCriteria = pgTable(
  "headcount_criteria",
  {
    id: serial().primaryKey().notNull(),
  ...
  (table) => [
    ...
    unique(
      "headcount_criteria_standard_id_property_id_min_value_max_va_key",
    ).on(table.standardId, table.propertyId, table.minValue, table.maxValue),
    check(
      "headcount_criteria_condition_operator_check",
      sql`(condition_operator)::text = ANY ((ARRAY['='::character varying, '<'::character varying, '<='::character varying, '>'::character varying, '>='::character varying, 'BETWEEN'::character varying])::text[])`,
    ),
  ],
)

export const headcountMonthlyFactors = pgTable(
  "headcount_monthly_factors",
  {
    id: serial().primaryKey().notNull(),
    standardId: integer("standard_id").notNull(),
  ...
  (table) => [
    ...
    unique(
      "headcount_monthly_factors_standard_id_duration_months_month_key",
    ).on(table.standardId, table.durationMonths, table.monthNo),
  ],
)
...
export const users = pgTable(
  ...
  (table) => [
    ...
    check(
      "users_status_check",
      sql`(status)::text = ANY ((ARRAY['ACTIVE'::character varying, 'INACTIVE'::character varying, 'LOCKED'::character varying])::text[])`,
    ),
    check(
      "users_system_role_check",
      sql`(system_role)::text = ANY ((ARRAY['USER'::character varying, 'SUPER_ADMIN'::character varying])::text[])`,
    ),
  ],
)
...
export const roles = pgTable(
  "roles",
  {
    ...
    planningMethod: varchar("planning_method", { length: 20 }).default(
      "BY_PROJECT",
    ),
    description: text(),
    createdAt: timestamp("created_at", { mode: "string" })
  ...
  (table) => [
    ...
    check(
      "roles_planning_method_check",
      sql`(planning_method)::text = ANY ((ARRAY['BY_SECTOR'::character varying, 'BY_REGION'::character varying, 'BY_PROJECT'::character varying])::text[])`,
    ),
  ],
)
...
export const sectors = pgTable(
  ...
  (table) => [unique("sectors_code_key").on(table.code)],
)
...
export const milestones = pgTable(
  ...
  (table) => [unique("milestones_code_key").on(table.code)],
)
